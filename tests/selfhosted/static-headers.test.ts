...
// Pins the precompressed (.br) header contract served by selfhosted/server.ts:
...
// stay immutably cacheable. The server self-executes on import (getDb +
// server.listen), so it is exercised over an ephemeral port instead of
// importing its helpers directly.
...
function createExportFixture(): { root: string; outDir: string } {
  const root = mkdtempSync(path.join(tmpdir(), "agent-render-static-headers-"));
...
describe("selfhosted precompressed header contract", () => {
