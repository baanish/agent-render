...
- `diff` uses `@pierre/diffs` for Shiki-backed unified and split review views. Multi-file patches add path-aware navigation through `@pierre/trees`; single-file diffs skip the tree. The artifact editor shows the same tree for multi-file patch editing and moves the patch textarea caret to the selected file's `diff --git` section.
