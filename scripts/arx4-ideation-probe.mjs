...
w();
w("Takeaway: host + label overhead is only ~20–50 chars. The real ceiling is ~3.8–3.9 KB of");
w("compressed bytes under baseBMP, or ~4.9 KB if a denser Unicode wire survives Discord's");
w("character counting as one unit per code point.");
w();
...
w(`| baseAstral (code points) | ${density.baseAstral1M.toFixed(2)} | ~${((density.baseAstral1M / density.baseBMP - 1) * 100).toFixed(0)}% denser **if** Discord counts code points |`);
w(`| baseAstral (UTF-16 units) | ${density.baseAstralUtf16Effective.toFixed(2)} | **Worse** than baseBMP if length is UTF-16 |`);
w();
w("**Critical unknown:** Discord's message length appears to count Unicode code points for");
w("most text, but markdown link destinations and client paste paths need an empirical check");
w("before shipping an astral wire. If any hop counts UTF-16, astral loses to baseBMP.");
w();
w("## Ideas probed");
w();
w("1. **baseAstral wire** — pack into supplementary-plane scalars (~20 bits/code point).");
w("2. **CBOR-ish binary tuple** — drop JSON quotes/escapes around the ARX2/3 tuple.");
...
w("   `brotli(dict‖data)−brotli(dict)` estimate and (b) a real `deflateRaw+dictionary` proxy.");
w("4. **Content-first binary envelope** — compress raw artifact bytes + tiny binary header, skip JSON entirely.");
...
w("7. **Discord framing** — short host + 1-char label + compact tag (cheap, orthogonal).");
w();
w("## Corpus results");
w();
...
w();
w("**Reading:** for Discord, ARX3 already leaves a lot of headroom on typical artifacts. The");
w("ways to *raise the ceiling* are almost entirely (1) denser Unicode wire and (2) fewer");
w("compressed bytes via shared dictionaries / better envelopes — not more text substitutions.");
w();
...
w("  and Brotli carries most of the work — and even then, baseAstral is the cleanest capacity");
w("  unlock if Discord counting cooperates.");
w();
w("### Highest-leverage ARX4 directions");
w();
w("1. **Validate Discord length semantics, then consider baseAstral**");
w("   - Pure wire change on top of identical ARX3 bytes.");
w("   - ~20–26% fewer visible units *and* ~25% more raw capacity *if* Discord counts code points.");
w("   - Zero win (or a loss) if any hop counts UTF-16 code units.");
w("   - Empirically test: paste a `#d` + astral payload link into Discord desktop/mobile/web.");
w();
w("2. **Content-first / CBOR binary envelope (skip JSON)**");
w("   - Alone: small (~0–5%) on already-substituted ARX3 text.");
w("   - Combined with a shared-dict estimate: best corpus win here (~8% fewer BMP chars).");
w("   - Removes JSON escaping of newlines/quotes; more important for pathological quote-heavy bodies.");
w("   - Natural fit for Discord's single-artifact share path; multi-artifact can keep tuples.");
w();
w("3. **Brotli shared static dictionary**");
w("   - Node cannot set a Brotli custom dictionary; numbers are estimates / deflate proxies.");
w("   - Still worth pursuing if `brotli-wasm` gains dictionary APIs — small/medium payloads benefit most.");
w("   - Deflate+dict is a real shared-dict signal but often *loses* to plain Brotli on this corpus");
w("     (+17% bytes) — do not switch away from Brotli; only add a true Brotli dictionary.");
w();
w("4. **Mined / larger overlay dictionaries**");
...
w();
w("5. **Discord framing (orthogonal, ship anytime)**");
w("   - Short branded host + 1-char label recovers ~15–30 chars — small but free.");
w("   - Agents should emit `[x](https://short/#…)` when targeting Discord.");
w();
...
w("  → baseBMP (safe) or baseAstral (Discord-validated)");
w("  → compact tag `d` / `e`");
w("```");
w();
w("Selection policy: optimize `markdownLink.length` for a declared surface");
w("(`discord` | `visible` | `transport`) instead of only fragment length.");
w();
w("## Non-goals / traps");
w();
...
w("- Do not assume astral density without client paste tests.");
